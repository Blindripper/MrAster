...
const PLAYBOOK_FILTER_REASON_METADATA = {
  long_overextended: {
    labelKey: 'playbook.filters.reason.long_overextended',
    fallback: 'Overextended longs',
    descriptionKey: 'playbook.filters.reason.long_overextended.desc',
    descriptionFallback: 'Limits trades when RSI or ATR show stretched long conditions.',
  },
  trend_extension: {
    labelKey: 'playbook.filters.reason.trend_extension',
    fallback: 'Trend extensions',
    descriptionKey: 'playbook.filters.reason.trend_extension.desc',
    descriptionFallback: 'Controls how many bars a momentum leg can extend before pausing trades.',
  },
  continuation_pullback: {
    labelKey: 'playbook.filters.reason.continuation_pullback',
    fallback: 'Continuation pullbacks',
    descriptionKey: 'playbook.filters.reason.continuation_pullback.desc',
    descriptionFallback: 'Shapes how deep pullbacks may go before the bot re-enters.',
  },
  spread_tight: {
    labelKey: 'playbook.filters.reason.spread_tight',
    fallback: 'Spread guard',
    descriptionKey: 'playbook.filters.reason.spread_tight.desc',
    descriptionFallback: 'Keeps bid/ask spreads within a tighter band.',
  },
  short_trend_alignment: {
    labelKey: 'playbook.filters.reason.short_trend_alignment',
    fallback: 'Short-term alignment',
    descriptionKey: 'playbook.filters.reason.short_trend_alignment.desc',
    descriptionFallback: 'Requires stronger agreement between short-term trend signals.',
  },
  edge_r: {
    labelKey: 'playbook.filters.reason.edge_r',
    fallback: 'Edge guard',
    descriptionKey: 'playbook.filters.reason.edge_r.desc',
    descriptionFallback: 'Raises the minimum edge required before trades fire.',
  },
  no_cross: {
    labelKey: 'playbook.filters.reason.no_cross',
    fallback: 'RSI crossovers',
    descriptionKey: 'playbook.filters.reason.no_cross.desc',
    descriptionFallback: 'Adjusts RSI crossover thresholds to stay aligned with regime bias.',
  },
  wicky: {
    labelKey: 'playbook.filters.reason.wicky',
    fallback: 'Wickiness filter',
    descriptionKey: 'playbook.filters.reason.wicky.desc',
    descriptionFallback: 'Filters symbols showing excessive wickiness or erratic prints.',
  },
  stoch_rsi_trend_short: {
    labelKey: 'playbook.filters.reason.stoch_rsi_trend_short',
    fallback: 'Stoch RSI trend',
    descriptionKey: 'playbook.filters.reason.stoch_rsi_trend_short.desc',
    descriptionFallback: 'Tunes stochastic RSI requirements for short-term confirmation.',
  },
  sentinel_veto: {
    labelKey: 'playbook.filters.reason.sentinel_veto',
    fallback: 'Sentinel veto',
    descriptionKey: 'playbook.filters.reason.sentinel_veto.desc',
    descriptionFallback: 'Sets event-risk gates and multipliers from the sentinel.',
  },
  playbook_structured_block: {
    labelKey: 'playbook.filters.reason.playbook_structured_block',
    fallback: 'Structured blocks',
    descriptionKey: 'playbook.filters.reason.playbook_structured_block.desc',
    descriptionFallback: 'Applies structured event-risk blocks that throttle trades.',
  },
};

const PLAYBOOK_FILTER_FIELD_METADATA = {
  rsi_cap: {
    labelKey: 'playbook.filters.field.rsi_cap',
    fallback: 'RSI cap',
    format: { type: 'index', digits: 0 },
  },
  atr_pct_cap: {
    labelKey: 'playbook.filters.field.atr_pct_cap',
    fallback: 'ATR % cap',
    format: { type: 'percent', digits: 2 },
  },
  bars_soft: {
    labelKey: 'playbook.filters.field.bars_soft',
    fallback: 'Soft bars',
    format: { type: 'integer' },
  },
  bars_hard: {
    labelKey: 'playbook.filters.field.bars_hard',
    fallback: 'Hard bars',
    format: { type: 'integer' },
  },
  adx_min: {
    labelKey: 'playbook.filters.field.adx_min',
    fallback: 'ADX min',
    format: { type: 'index', digits: 0 },
  },
  stoch_warn: {
    labelKey: 'playbook.filters.field.stoch_warn',
    fallback: 'Stoch warn',
    format: { type: 'index', digits: 0 },
  },
  stoch_max: {
    labelKey: 'playbook.filters.field.stoch_max',
    fallback: 'Stoch max',
    format: { type: 'index', digits: 0 },
  },
  stoch_min: {
    labelKey: 'playbook.filters.field.stoch_min',
    fallback: 'Stoch min',
    format: { type: 'index', digits: 0 },
  },
  adx_delta_min: {
    labelKey: 'playbook.filters.field.adx_delta_min',
    fallback: 'ADX Δ min',
    format: { type: 'index', digits: 1 },
  },
  min_edge_r: {
    labelKey: 'playbook.filters.field.min_edge_r',
    fallback: 'Min edge R',
    format: { type: 'ratio', digits: 2, suffix: ' R' },
  },
  spread_bps_max: {
    labelKey: 'playbook.filters.field.spread_bps_max',
    fallback: 'Max spread',
    format: { type: 'bps', digits: 1 },
  },
  wickiness_max: {
    labelKey: 'playbook.filters.field.wickiness_max',
    fallback: 'Max wickiness',
    format: { type: 'ratio', digits: 3 },
  },
  rsi_buy_min: {
    labelKey: 'playbook.filters.field.rsi_buy_min',
    fallback: 'RSI buy min',
    format: { type: 'index', digits: 0 },
  },
  rsi_sell_max: {
    labelKey: 'playbook.filters.field.rsi_sell_max',
    fallback: 'RSI sell max',
    format: { type: 'index', digits: 0 },
  },
  slope_min: {
    labelKey: 'playbook.filters.field.slope_min',
    fallback: 'Slope min',
    format: { type: 'percent', digits: 2 },
  },
  supertrend_tol: {
    labelKey: 'playbook.filters.field.supertrend_tol',
    fallback: 'Supertrend tolerance',
    format: { type: 'signed', digits: 2 },
  },
  event_risk_gate: {
    labelKey: 'playbook.filters.field.event_risk_gate',
    fallback: 'Event risk gate',
    format: { type: 'percent', digits: 0 },
  },
  block_risk: {
    labelKey: 'playbook.filters.field.block_risk',
    fallback: 'Block risk',
    format: { type: 'percent', digits: 0 },
  },
  min_multiplier: {
    labelKey: 'playbook.filters.field.min_multiplier',
    fallback: 'Min multiplier',
    format: { type: 'multiplier', digits: 2 },
  },
  weight: {
    labelKey: 'playbook.filters.field.weight',
    fallback: 'Risk weight',
    format: { type: 'multiplier', digits: 2 },
  },
  event_risk_max: {
    labelKey: 'playbook.filters.field.event_risk_max',
    fallback: 'Event risk cap',
    format: { type: 'percent', digits: 0 },
  },
  soft_multiplier: {
    labelKey: 'playbook.filters.field.soft_multiplier',
    fallback: 'Soft multiplier',
    format: { type: 'multiplier', digits: 2 },
  },
};
...
function getPlaybookFilterFieldMeta(field) {
  const normalized = (field || '').toString().trim().toLowerCase();
  return PLAYBOOK_FILTER_FIELD_METADATA[normalized];
}

function getPlaybookFilterFieldLabel(field) {
  const meta = getPlaybookFilterFieldMeta(field);
  if (meta) {
    return translate(meta.labelKey, meta.fallback);
  }
  const label = (field || '').toString().replace(/_/g, ' ').trim();
  return label ? toTitleCase(label) : '';
}

function getPlaybookFilterReasonLabel(reason) {
  const normalized = (reason || '').toString().trim().toLowerCase();
  const meta = PLAYBOOK_FILTER_REASON_METADATA[normalized];
  if (meta) {
    return translate(meta.labelKey, meta.fallback);
  }
  const label = (reason || '').toString().replace(/_/g, ' ').trim();
  return label ? toTitleCase(label) : '';
}

function getPlaybookFilterReasonDescription(reason) {
  const normalized = (reason || '').toString().trim().toLowerCase();
  const meta = PLAYBOOK_FILTER_REASON_METADATA[normalized];
  if (!meta) return null;
  if (!meta.descriptionKey && !meta.descriptionFallback) return null;
  return translate(meta.descriptionKey, meta.descriptionFallback);
}

function formatPlaybookFilterValue(field, value) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return null;
  const meta = getPlaybookFilterFieldMeta(field);
  const spec = meta?.format || {};
  const digits = Number.isFinite(spec.digits) ? spec.digits : undefined;
  let formatted;
  switch (spec.type) {
    case 'percent': {
      const pctDigits = digits ?? 0;
      formatted = `${(numeric * 100).toFixed(pctDigits)}%`;
      break;
    }
    case 'bps': {
      const bpsDigits = digits ?? 1;
      formatted = `${(numeric * 10000).toFixed(bpsDigits)} bps`;
      break;
    }
    case 'multiplier': {
      const multDigits = digits ?? 2;
      formatted = `×${numeric.toFixed(multDigits)}`;
      break;
    }
    case 'integer': {
      formatted = `${Math.round(numeric)}`;
      break;
    }
    case 'index': {
      const idxDigits = digits ?? 0;
      formatted = `${numeric.toFixed(idxDigits)}`;
      break;
    }
    case 'signed': {
      const signedDigits = digits ?? 2;
      const sign = numeric >= 0 ? '+' : '';
      formatted = `${sign}${numeric.toFixed(signedDigits)}`;
      break;
    }
    case 'ratio': {
      const ratioDigits = digits ?? (Math.abs(numeric) >= 1 ? 2 : 3);
      formatted = `${numeric.toFixed(ratioDigits)}`;
      break;
    }
    default: {
      const defaultDigits = digits ?? (Math.abs(numeric) >= 1 ? 2 : 3);
      formatted = `${numeric.toFixed(defaultDigits)}`;
      break;
    }
  }
  if (!formatted) return null;
  if (spec.suffix) {
    return `${formatted}${spec.suffix}`;
  }
  return formatted;
}

function normalizePlaybookFilterOverrides(filters) {
  if (!filters || typeof filters !== 'object') return [];
  const entries = [];
  Object.entries(filters).forEach(([reason, values]) => {
    if (!values || typeof values !== 'object') return;
    const adjustments = [];
    Object.entries(values).forEach(([field, rawValue]) => {
      const valueText = formatPlaybookFilterValue(field, rawValue);
      if (!valueText) return;
      const label = getPlaybookFilterFieldLabel(field);
      if (!label) return;
      adjustments.push({ field, label, value: valueText });
    });
    if (!adjustments.length) return;
    adjustments.sort((a, b) => a.label.localeCompare(b.label, undefined, { sensitivity: 'base' }));
    const label = getPlaybookFilterReasonLabel(reason);
    const description = getPlaybookFilterReasonDescription(reason);
    entries.push({ reason, label: label || reason, description, adjustments });
  });
  entries.sort((a, b) => a.label.localeCompare(b.label, undefined, { sensitivity: 'base' }));
  return entries;
}

function buildPlaybookFilterSection(entries) {
  if (!Array.isArray(entries) || entries.length === 0) return null;
  const section = document.createElement('div');
  section.className = 'playbook-filters';

  const title = document.createElement('div');
  title.className = 'playbook-filters-title';
  title.textContent = translate('playbook.filters.title', 'Trade filter adjustments');
  section.append(title);

  const subtitle = document.createElement('p');
  subtitle.className = 'playbook-filters-subtitle';
  subtitle.textContent = translate(
    'playbook.filters.subtitle',
    'AI tightened the guardrails below to match current market conditions.'
  );
  section.append(subtitle);

  const list = document.createElement('div');
  list.className = 'playbook-filter-list';
  entries.forEach((entry) => {
    const card = document.createElement('article');
    card.className = 'playbook-filter-card';

    const header = document.createElement('div');
    header.className = 'playbook-filter-card__header';
    header.textContent = entry.label;
    card.append(header);

    if (entry.description) {
      const description = document.createElement('p');
      description.className = 'playbook-filter-card__description';
      description.textContent = entry.description;
      card.append(description);
    }

    const chips = document.createElement('div');
    chips.className = 'playbook-filter-card__chips';
    entry.adjustments.forEach((adjustment) => {
      const chip = document.createElement('span');
      chip.className = 'playbook-filter-chip';
      chip.textContent = `${adjustment.label}: ${adjustment.value}`;
      chips.append(chip);
    });
    card.append(chips);
    list.append(card);
...
function createPlaybookSummaryContent(state, { hint } = {}) {
  ...
  const filterEntries = normalizePlaybookFilterOverrides(state.filters);
  const filterSection = buildPlaybookFilterSection(filterEntries);
  if (filterSection) {
    fragment.append(filterSection);
  }
