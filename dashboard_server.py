...
def _get_exchange_base() -> str:
    try:
        base = CONFIG.get("env", {}).get("ASTER_EXCHANGE_BASE")  # type: ignore[name-defined]
    except NameError:
        base = None
    if not base:
        base = os.getenv("ASTER_EXCHANGE_BASE", ENV_DEFAULTS["ASTER_EXCHANGE_BASE"])
    return base.rstrip("/")
...
def _fetch_most_traded_from_exchange(limit: int = 8) -> List[Dict[str, Any]]:
    resp = requests.get(_exchange_url("/fapi/v1/ticker/24hr"), timeout=8)
    resp.raise_for_status()
    payload = resp.json()
...
async def get_most_traded_assets(force: bool = False) -> Dict[str, Any]:
    ...
    try:
        assets = await asyncio.to_thread(_fetch_most_traded_from_exchange)
    except Exception:
        if cached_payload:
            return cached_payload
