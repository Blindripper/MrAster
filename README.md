...
- `ASTER_AI_NEWS_ENDPOINT`: Optional HTTP endpoint that returns the latest headlines for a symbol. A practical source is a small
  proxy around an aggregator such as the CryptoPanic `posts` API or a self-hosted feed that normalises Binance/OKX announcement
  RSS into JSON.
